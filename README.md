...
7. **Weekly reflection** — per app, what you said when opening it next to how the pause's
...
8. **Settings** — a short menu grouped by what each setting affects: the question when you open
...
Two more are **optional**, and each only matters if you turn on the feature that uses it:
...
| `RECEIVE_BOOT_COMPLETED` | ours | Re-arm the daily maintenance and weekly reflection jobs after a reboot, and undo grayscale if the phone restarted while it was on |
